...
Nb :  Promo pendaftaran 57 ribu hanya berlaku sampai 4 Agustus 2024, setelah itu biaya pendaftaran akan naik jadi 123 ribu';
    ...
    public function pemesanan_kelasprofit_plus(Request $request)
    {
        $agen = 100188;
        $page_title = 'Kelas entrepreneurID | Kelas Profit 10 Juta';
        SEO::title($page_title)
        ...
        return view('pages.pemesanan-plus', compact('agen'));
    }

    public function pemesanan_kelasprofit_plus_store($agen, Request $request)
    {
        if (!Auth::check()) {
            $request->validate([
                'name'           => ['required', 'string', 'max:255'],
